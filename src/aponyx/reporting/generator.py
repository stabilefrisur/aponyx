...
def _collect_report_data(signal_name: str, strategy_name: str) -> ReportData:
    ...
    performance_files = list(
        PERFORMANCE_REPORTS_DIR.glob(f"{signal_name}_{strategy_name}_*.md")
    )
    ...
    viz_dir = (
        ...
        / "workflows"
        / "visualizations"
        / f"{signal_name}_{strategy_name}"
    )
    data.has_visualizations = viz_dir.exists() and any(viz_dir.glob("*.html"))
    
    # Find workflow directory
    workflow_dirs = list(
        PROCESSED_DIR.glob(f"workflows/{signal_name}_{strategy_name}_*")
    )
    if workflow_dirs:
...
def _generate_html_report(data: ReportData) -> str:
    ...
    html_parts.append(f"    <title>Research Report: {data.signal_name} ({data.strategy_name})</title>")
    html_parts.append("    <style>")
    html_parts.append("        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 1200px; margin: 40px auto; padding: 0 20px; line-height: 1.6; }")
    html_parts.append("        h1 { border-bottom: 3px solid #333; padding-bottom: 10px; }")
    html_parts.append("        h2 { border-bottom: 1px solid #ccc; padding-bottom: 5px; margin-top: 40px; }")
    ...
    if data.has_visualizations:
        ...
        for viz_file in sorted(viz_dir.glob("*.html")):
            html_parts.append(f"        <li><a href=\"{viz_file.resolve().as_uri()}\" target=\"_blank\">{viz_file.stem}</a></li>")
        html_parts.append("    </ul>")
    
    # Workflow info
    if data.workflow_dir:
        html_parts.append("    <h2>Workflow Details</h2>")
        html_parts.append(f"    <p><strong>Output Directory:</strong> <code>{data.workflow_dir}</code></p>")
    ...
    for line in markdown_text.split("\n"):
        ...
        # Include table content (simplified)
            ...
            if line.strip() and not line.startswith("**Generated"):
                if any(keyword in line.lower() for keyword in ["decision:", "score:", "sharpe", "return", "drawdown"]):
