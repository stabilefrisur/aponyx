...
> **Auto-generated from codebase analysis** | Last Updated: December 20, 2025
...
# Testing & Quality
uv run pytest              # Run all tests (900+ tests)
...
**Patterns**:
- Signal-based position triggers with asymmetric entry/exit
  - Entry: |signal| >= entry_threshold (default varies by strategy: 1.0-1.8)
  - Exit: signal returns to zero (via signal transformation's neutral_range)
  - Enables mean-reversion strategies where position runs before closing
- Binary and proportional sizing modes
- PnL-based exits with cooldown (stop_loss_pct, take_profit_pct)
- ReturnCalculator abstraction: SpreadReturnCalculator (DV01) or PriceReturnCalculator (price returns)
- Transaction costs: static (bps) or dynamic (spread percentage)
...
### 4. Data Channel Pattern (CRITICAL - New in 0.1.20)
...
```python
# Step N produces output (e.g., SignalStep)
def execute(self, context: dict[str, Any]) -> dict[str, Any]:
    # Save signal and intermediates
    save_parquet(signal, output_dir / "signal.parquet")
    save_parquet(indicator, output_dir / "indicator.parquet")
    save_parquet(score, output_dir / "score.parquet")
    
    return {"signal": signal, "indicator": indicator, "score": score}

# Step N+1 consumes it (e.g., VisualizationStep)
def execute(self, context: dict[str, Any]) -> dict[str, Any]:
    signal = context["signal"]["signal"]
